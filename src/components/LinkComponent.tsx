...
export function LinkComponent (props: Props) {
  ...
  if (isExternal) {
    return (
      <Link className={className} _hover={{ color }} href={props.href} target="_blank" rel="noopener noreferrer">
        {props.children}
      </Link>
    )
  }
  ...
  return (
    <Link as={NextLink} className={className} _hover={{ color }} href={props.href}>
      {props.children}
    </Link>
  )
}
